...
# Rolling window: past week + next 6 months
TODAY = dt.date.today()
...
def fetch_knicks() -> list[dict]:
    # ESPN's team schedule endpoint returns the current/upcoming season.
    # We also pull next season if we're late in the year so the window stays full.
    seasons = [TODAY.year]
    if TODAY.month >= 7:  # NBA season-year is the year it ENDS; July+ → next season exists
    ...
    for season in seasons:
        ...
        try:
            ...
            print(f"  warn: knicks season {season} fetch failed: {e}")
            continue
